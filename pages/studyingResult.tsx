...
async function done(userId, isDone){
  const db = firebase.firestore();
  // userのカレントスタディのreferenceを取得
  const user = await db.collection("users").doc(userId).get();
  const currentStudyRef = user.get("currentStudy");
  await currentStudyRef.update("isDone" , isDone);
  const userRef = db.collection("users").doc(userId);
  // カレントスタディをnullにする
  await userRef.update("currentStudy", null);
...
const StudyingResult = () => {
  ...
  <div className="wrapper book">
    <div className="main">
      <p>まるまるできましたか？</p>
      <button className="finishStudy" onClick={() => done(userId, true)}>Yes!</button>
      <button className="finishStudy" onClick={() => done(userId, false)}>No</button>
    </div>
  </div>
  )
};

export default StudyingResult;